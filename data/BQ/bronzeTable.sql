CREATE EXTERNAL TABLE IF NOT EXISTS `prathamdev.bronze_dataset.orders`(
    order_id INT64,
    customer_id INT64,
    order_date STRING,
...
OPTIONS (
    format = 'JSON',
    uris = ['gs://datalake-project/landing/retailer-db/order_items/*.json']
);
-------------------------------------------------------------------------------------------------------------
-- Suppliers Table
CREATE EXTERNAL TABLE IF NOT EXISTS `prathamdev.bronze_dataset.suppliers` (
    supplier_id INT64,
...
OPTIONS (
  format = 'JSON',
  uris = ['gs://datalake-project/landing/supplier-db/suppliers/*.json']
);

-- Product Suppliers Table (Mapping suppliers to products)
CREATE EXTERNAL TABLE IF NOT EXISTS `prathamdev.bronze_dataset.product_suppliers` (
    supplier_id INT64,
    product_id INT64,
...
OPTIONS (
  format = 'JSON',
  uris = ['gs://datalake-project/landing/supplier-db/product_suppliers/*.json']
);

-------------------------------------------------------------------------------------------------------------

CREATE OR REPLACE EXTERNAL TABLE `prathamdev.bronze_dataset.customer_reviews` (
  id STRING,
  customer_id INT64,
  product_id INT64,
  rating INT64,
...
OPTIONS (
  format = 'PARQUET',
  uris = ['gs://datalake-project/landing/customer_reviews/customer_reviews_*.parquet']
);
