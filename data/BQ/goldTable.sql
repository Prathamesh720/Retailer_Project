...
FROM `prathamdev.silver_dataset.orders` o
JOIN `prathamdev.silver_dataset.order_items` oi ON o.order_id = oi.order_id
JOIN `prathamdev.silver_dataset.products` p ON oi.product_id = p.product_id
JOIN `prathamdev.silver_dataset.categories` c ON p.category_id = c.category_id
...
FROM `prathamdev.silver_dataset.customers` c
LEFT JOIN `prathamdev.silver_dataset.orders` o ON c.customer_id = o.customer_id
LEFT JOIN `prathamdev.silver_dataset.order_items` oi ON o.order_id = oi.order_id
...
LEFT JOIN `prathamdev.silver_dataset.order_items` oi ON p.product_id = oi.product_id
LEFT JOIN `prathamdev.silver_dataset.customer_reviews` cr ON p.product_id = cr.product_id
...
SELECT 
    ...
    SUM(oi.quantity) AS total_units_sold,
    SUM(oi.price * oi.quantity) AS total_revenue
FROM `prathamdev.silver_dataset.suppliers` s
LEFT JOIN `prathamdev.silver_dataset.product_suppliers` ps ON s.supplier_id = ps.supplier_id
LEFT JOIN `prathamdev.silver_dataset.order_items` oi ON ps.product_id = oi.product_id
